...
def category_stats():
    # return Category.query.join(Product, Product.category_id.__eq__(Category.id), isouter=True).add_columns(
    #     func.count(Product.id)).group_by(Category.id, Category.name).all()

    return db.session.query(Category.id, Category.name, func.count(Product.id)).join(Product, Category.id.__eq__(
        Product.category_id), isouter=True).group_by(Category.id, Category.name).all()


def product_stats(kw=None, from_date=None, to_date=None):
    p = db.session.query(Product.id, Product.name, func.sum(ReceiptDetail.quantity * ReceiptDetail.unit_price)).join(
        ReceiptDetail, ReceiptDetail.product_id.__eq__(Product.id), isouter=True).join(Receipt, Receipt.id.__eq__(
        ReceiptDetail.receipt_id)).group_by(Product.id, Product.name)
...
def product_month_stats(year):
    return db.session.query(extract('month', Receipt.created_date),
                            func.sum(ReceiptDetail.quantity * ReceiptDetail.unit_price)).join(
        ReceiptDetail, ReceiptDetail.receipt_id.__eq__(Receipt.id)
        ).filter(extract('year', Receipt.created_date) == year).group_by(
        extract('month', Receipt.created_date)).order_by(extract('month', Receipt.created_date)).all()
